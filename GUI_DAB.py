...
def enter_money(num_compte, solde_actual):
    ...
    if not montant:
        ...
        try:
            swared = float(montant)
        except ValueError:
            ecran.insert(INSERT, '\n\n\n Tapez un montant SVP.')
        else:
            if swared <= solde_actual:
                solde.set(str(solde_actual - swared))
                db.retirer(num_compte, swared)
                ecran.insert(INSERT, '\n\n\n Opération réussie.')
...
code_entry = Entry(first_frame, bg='pale green',
                   ...
                   validate=ALL, validatecommand=(v_cmd, '%P'))
